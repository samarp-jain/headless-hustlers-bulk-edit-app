...
const getSaveConfigOptions = (configInputFields: any) => {
  ...
  Object.keys(configInputFields)?.forEach((field: string) => {
    if (configInputFields?.[field]?.saveInConfig) saveInConfig[field] = configInputFields[field];
    if (configInputFields?.[field]?.saveInServerConfig) saveInServerConfig[field] = configInputFields[field];
  });
...
const getDefaultInputValues = (configInputFields: any) => {
  const { saveInConfig, saveInServerConfig } = getSaveConfigOptions(configInputFields);

  const radioValuesKeys = [
    ...(Object.keys(saveInConfig)?.filter((value) => saveInConfig?.[value]?.type === "radioInputFields") ?? []),
    ...(Object.keys(saveInServerConfig)?.filter((value) => saveInServerConfig?.[value]?.type === "radioInputFields") ??
      []),
  ];

  const selectValuesKeys = [
    ...(Object.keys(saveInConfig)?.filter((value) => saveInConfig?.[value]?.type === "selectInputFields") ?? []),
    ...(Object.keys(saveInServerConfig)?.filter((value) => saveInServerConfig?.[value]?.type === "selectInputFields") ??
      []),
  ];
...
const getIntialValueofComponents = ({ savedData, radioValuesKeys, selectValuesKeys, configInputFields }: any) => {
